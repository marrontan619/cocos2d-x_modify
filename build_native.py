...
def check_environment_variables(target):
    ...
    try:
        ret = os.environ[target]
    except Exception:
        print target + " is not defined. Please define " + target + " in your environment"
        sys.exit(1)
...
def do_build(cocos_root, ndk_root, app_android_root,ndk_build_param,sdk_root,android_platform,build_mode):
    ...
    if os.system(command) != 0:
        ...
        sdk_tool_path = os.path.join(sdk_root, "tools/android.bat")
...
def copy_resources(app_android_root, cocos_root):
    ...
    resources_dir = os.path.join(cocos_root, "cocos/scripting/javascript/script")
    copy_files(resources_dir, assets_dir)

def build(ndk_build_param,android_platform,build_mode):
    current_dir = os.path.dirname(os.path.realpath(__file__))
    cocos_root = check_environment_variables("COCOS2DX")

    ndk_root = check_environment_variables("NDK_ROOT")
    sdk_root = None
    select_toolchain_version(ndk_root)


    app_android_root = current_dir
    copy_resources(app_android_root, cocos_root)
    
    if android_platform is not None:
        sdk_root = check_environment_variables("ANDROID_SDK_ROOT")
